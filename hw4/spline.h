...
// b-spline
template <typename T>
inline T b_spline(T p0, T p1, T p2, T p3, float t) {
    float b0 =
...
template <>
inline glm::quat b_spline(glm::quat p0, glm::quat p1, glm::quat p2, glm::quat p3, float t) {
    float b0 =
        (1.0f / 6.0f) * (1.0f - t) * (1.0f - t) * (1.0f - t);
    float b1 =
        (1.0f / 6.0f) * (3.0f * t * t * t - 6.0f * t * t + 4.0f);
...
// bezier curve
template <typename T>
inline T bezier_curve(T p0, T p1, T p2, T p3, float t) {
...
template <>
inline glm::quat bezier_curve(glm::quat p0, glm::quat p1, glm::quat p2, glm::quat p3, float t) {
    float b0 = (1.0f - t) * (1.0f - t) * (1.0f - t);
    float b1 = 3.0f * t * (1.0f - t) * (1.0f - t);
    float b2 = 3.0f * t * t * (1.0f - t);
    float b3 = t * t * t;
...
template <typename T>
inline T catmullrom_spline(T p0, T p3, T tgt0, T tgt3, float t) {
...
template <>
inline glm::quat catmullrom_spline(glm::quat p0, glm::quat p3, glm::quat tgt0, glm::quat tgt3, float t) {
    // using hermite to bezier transform & affine invariance
